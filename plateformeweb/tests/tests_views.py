...
class TestAllViews(TestCase):
    def setUp(self):
        ...
        self.placetype = PlaceType.objects.create(
            name="repaircafe", slug="repaircafe")
        self.activity = Activity.objects.create(
            name="cafe", owner=self.admin, picture="static/img/event-card.jpg"
        )
    ...
    def test_organization_person_user_views(self):
        self.org_person = OrganizationPerson.objects.create(
        ...
        authorized_views = [
            ...
            reverse(
                "activity_detail",
                kwargs={"pk": self.activity.pk, "slug": self.activity.slug},
            ),
            reverse("event_list"),
            reverse(
                "event_detail", kwargs={
    ...
    def test_organization_person_with_volunteer_role_user_views(self):
        self.volunteer = CustomUser.objects.create_user(
            "bourguiba", "password")
        self.org_person = OrganizationPerson.objects.create(
            user=self.volunteer, organization=self.organization, role=20
        )
