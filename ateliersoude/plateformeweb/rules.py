    ...
    try:
        if isinstance(obj, Organization):
            ...
            raise AppError(
                ...
                "field", obj, )
        if org_member_class.objects.filter(
                user=user).filter(
...
rules.add_perm(
    "plateformeweb.edit_organization",
    is_org_admin_for_object | rules.is_staff | rules.is_superuser,
)

# activities
rules.add_perm(
    "plateformeweb.create_activity",
    is_org_admin_for_object | rules.is_staff | rules.is_superuser,
)
rules.add_perm("plateformeweb.edit_activity", is_owner_for_object |
               is_org_admin_for_object | rules.is_staff | rules.is_superuser,)
rules.add_perm(
    "plateformeweb.delete_activity",
    is_org_admin_for_object | rules.is_staff | rules.is_superuser,
)

# events
rules.add_perm(
    "plateformeweb.create_event",
    is_org_admin_for_object | rules.is_staff | rules.is_superuser,
)

rules.add_perm("plateformeweb.edit_event", is_owner_for_object |
               is_org_admin_for_object | rules.is_staff | rules.is_superuser,)
rules.add_perm(
    "plateformeweb.delete_event",
    is_org_admin_for_object | rules.is_staff | rules.is_superuser,
)

# places
rules.add_perm(
    "plateformeweb.create_place",
    is_org_admin_for_object | rules.is_staff | rules.is_superuser,
)
rules.add_perm("plateformeweb.edit_place", is_owner_for_object |
               is_org_admin_for_object | rules.is_staff | rules.is_superuser,)
rules.add_perm(
    "plateformeweb.delete_place",
    is_org_admin_for_object | rules.is_staff | rules.is_superuser,
)
