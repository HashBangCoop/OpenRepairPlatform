    ...
    def handle(self, *args, **options):
        ...
        for event in unpublished_events:
            ...
            for user in orga.volunteers.all() | orga.admins.all():
                event_url = base_url + reverse("event:detail", kwargs={
                    "pk": event.id,
                    "slug": event.slug,
                })
                msg_plain = render_to_string("event/mail/event_published.txt",
                                             context=locals())
                msg_html = render_to_string("event/mail/event_published.html",
                                            context=locals())
                send_mail(
