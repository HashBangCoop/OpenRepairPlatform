...
def test_add_admin_to_organization_forbidden(client, user_log, organization):
    ...
    response = client.post(
        reverse("user:organization_add_admin", kwargs={
...
def test_add_admin_to_organization(custom_user_factory, client, organization):
    ...
    assert response.status_code == 302
    assert response.url == reverse("user:organization_detail", kwargs={
        "pk": organization.pk,
        "slug": organization.slug,
    })
    admins = organization.admins.all()
    assert len(admins) == 2
    assert admins[0].pk == user.pk


def test_add_admin_to_organization_wrong_user(custom_user, client,
                                              organization):
...
def test_add_volunteer_to_organization_forbidden(client, user_log,
                                                 organization):
    client.login(email=user_log.email, password=USER_PASSWORD)
    response = client.post(
        reverse("user:organization_add_volunteer", kwargs={
            "orga_pk": organization.pk
        }),
        {"email": user_log.email},
    )
...
def test_add_volunteer_to_organization(custom_user_factory, client,
                                       organization):
    user = custom_user_factory()
    admin = custom_user_factory()
    organization.admins.add(admin)
...
def test_remove_volunteer_from_organization_forbidden(client, user_log,
                                                      organization):
    client.login(email=user_log.email, password=USER_PASSWORD)
    response = client.post(
        reverse("user:remove_from_volunteers", kwargs={
            "orga_pk": organization.pk,
...
def test_remove_volunteer_from_organization(custom_user_factory, client,
                                            organization):
    user = custom_user_factory()
    admin = custom_user_factory()
...
def test_remove_admin_from_organization(custom_user_factory, client,
                                        organization):
    user = custom_user_factory()
    admin = custom_user_factory()
    organization.admins.add(admin)
    ...
    response = client.post(
        reverse("user:remove_from_admins", kwargs={
            "orga_pk": organization.pk,
            "user_pk": user.pk,
        }),
    )
